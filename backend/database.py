...
class PostgreSQLManager:
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Create re-entry table for subsequent captures
                    cursor.execute('''
                        CREATE TABLE IF NOT EXISTS re_entry (
                            id SERIAL PRIMARY KEY,
                            person_name VARCHAR(255) NOT NULL,
                            image_url TEXT NOT NULL,
                            s3_key TEXT,
                    ...
                    cursor.execute('CREATE INDEX IF NOT EXISTS idx_re_entry_person_name ON re_entry(person_name)')
                    cursor.execute('CREATE INDEX IF NOT EXISTS idx_pass_in_capture_time ON pass_in(capture_time)')
                    cursor.execute('CREATE INDEX IF NOT EXISTS idx_re_entry_capture_time ON re_entry(capture_time)')
    ...
    def add_re_entry(self, person_name: str, image_url: str, s3_key: Optional[str] = None,
                          face_confidence: Optional[float] = None, pass_in_entry_id: Optional[int] = None) -> int:
        """Add entry to re_entry table (subsequent capture)."""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute('''
                        INSERT INTO re_entry (person_name, image_url, s3_key, face_confidence, pass_in_entry_id)
                        VALUES (%s, %s, %s, %s, %s)
                        RETURNING id
                    ''', (person_name, image_url, s3_key, face_confidence, pass_in_entry_id))
                    entry_id = cursor.fetchone()[0]
                    conn.commit()
                    logger.info(f"Added re_entry entry for {person_name} with ID: {entry_id}")
                    return entry_id
        except Exception as e:
            logger.error(f"Error adding re_entry entry: {e}")
            raise
    ...
    def get_all_re_entry_entries(self, person_name: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        """Get all entries from re_entry table."""
        try:
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                    if person_name:
                        cursor.execute('''
                            SELECT * FROM re_entry WHERE person_name = %s 
                            ORDER BY created_at DESC LIMIT %s
                        ''', (person_name, limit))
                    else:
                        cursor.execute('''
                            SELECT * FROM re_entry ORDER BY created_at DESC LIMIT %s
                        ''', (limit,))
                    
                    return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
            logger.error(f"Error getting re_entry entries: {e}")
            return []
    
    def get_re_entry_count(self, person_name: str) -> int:
        """Get the count of re_entry entries for a person today."""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Count re_entry entries for today
                    cursor.execute('''
            ...
            logger.error(f"Error getting re_entry count: {e}")
            return 0
    
    def get_unique_persons(self) -> List[str]:
        """Get list of unique person names from pass_in and re_entry tables."""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute('''
                        SELECT DISTINCT person_name FROM (
                            ...
                            UNION
                            SELECT person_name FROM re_entry
    ...
    def get_recent_captures(self, hours: int = 24) -> List[Dict[str, Any]]:
        """Get recent captures from pass_in and re_entry tables within specified hours."""
        try:
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                    ...
                    # Get recent re_entry entries
                    cursor.execute('''
                        SELECT 're_entry' as entry_type, * FROM re_entry 
                        WHERE created_at >= NOW() - INTERVAL '%s hours'
                        ORDER BY created_at DESC
                    ''', (hours,))
                    
                    re_entry_entries = [dict(row) for row in cursor.fetchall()]
                    
                    # Combine and sort by created_at
                    all_entries = pass_in_entries + re_entry_entries
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Get total re_entry entries
                    cursor.execute('SELECT COUNT(*) FROM re_entry')
                    total_re_entry = cursor.fetchone()[0]
                    
                    # Get total captures
                    total_captures = total_pass_in + total_re_entry
                    
                    return {
                        'total_persons': unique_persons,
                        'total_captures': total_captures,
                        'pass_in_count': total_pass_in,
                        're_entry_count': total_re_entry,
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    if table == 'pass_in':
                        ...
                        cursor.execute('DELETE FROM re_entry WHERE id = %s', (entry_id,))
                    else:
                        return False
